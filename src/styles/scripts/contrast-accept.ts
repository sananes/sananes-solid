...
import { AA_TEXT, APCA_MIN, BASELINE_PATH, measureContrast, readBaseline } from "./contrast"

const measurements = await measureContrast()
const failing = measurements.filter((m) => m.ratio < m.min)
const weakApca = measurements.filter((m) => m.min === AA_TEXT && Math.abs(m.lc) < APCA_MIN)
...
const apcaAdded = Object.keys(apcaAccepted).filter((k) => !(k in previous.apcaAccepted))
const apcaRemoved = Object.keys(previous.apcaAccepted).filter((k) => !(k in apcaAccepted))

writeFileSync(BASELINE_PATH, `${JSON.stringify({ accepted, apcaAccepted }, null, 2)}\n`)

if (failing.length === 0) {
  console.log("✓ Every measured pair meets WCAG AA. Baseline is empty.")
} else {
  console.log(`Accepting ${failing.length} pair(s) below WCAG AA — each one ships as-is:\n`)
...
if (removed.length > 0) console.log(`  no longer failing: ${removed.join(", ")}`)
...
if (apcaAdded.length > 0) console.log(`\n  newly accepted: ${apcaAdded.join(", ")}`)
if (apcaRemoved.length > 0) console.log(`  no longer failing: ${apcaRemoved.join(", ")}`)
