 ...
 *
 * Run with: bun test src/styles/scripts/setup-styles.test.ts
 */
...
const readCommitted = (path: string) => readFileSync(join(repoRoot, path), "utf8")

describe("generated stylesheets", () => {
  it("tailwind.css matches a fresh build of the current config", async () => {
    expect(readCommitted(OUTPUTS.tailwind)).toBe(built.tailwind)
  })
...
describe("color authoring rules", () => {
  ...
  //
  // Scoped to the styles system rather than the whole repo: the app's own
  // palette in `src/app.css` predates this port and is authored in hex. Widen
  // `roots` back to ['app', 'components', 'lib', 'src'] once that is converted.
  it("authors every colour in hand-written CSS as oklch", () => {
    ...
    const roots = ["src/styles"].map((d) => join(repoRoot, d))
    const cssFiles = roots.flatMap((r) => {
      try {
        return walk(r)
      } catch {
        return []
    ...
    for (const fullPath of cssFiles) {
      ...
      for (const { groups } of css.matchAll(DECLARATION)) {
        ...
        const value = rawValue.replace(/var\([^)]*\)/g, "").replace(/url\([^)]*\)/g, "")
