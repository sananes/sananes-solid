 ...
 * `src/styles/css/index.css`, reset second — so without the explicit
 ...
 *
 * On failure: someone reverted the layer setup in `src/styles/css/index.css`.
 * Put the `@layer base, utilities;` declaration and both `layer(...)` import
 * modifiers back.
 *
 * Run with: bun test src/styles/scripts/utility-layering.test.ts
 */
...
const indexCssPath = join(repoRoot, "src/styles/css/index.css")
...
async function compile() {
  // `index.css` imports Tailwind with `source(none)` — the app declares what to
  // scan in `src/app.css`. Compiling this file alone therefore finds no
  // candidates, so the two utilities these assertions look for are requested
  // explicitly. Keeps the check on layer order, not on file scanning.
...
describe("cascade layer order (source)", () => {
  ...
  it("imports tailwindcss/utilities.css with the 'utilities' layer modifier", async () => {
    const source = readFileSync(indexCssPath, "utf8")
    expect(source).toMatch(/@import\s+['"]tailwindcss\/utilities\.css['"]\s+layer\(utilities\)/)
  })
})
...
describe("cascade layer order (compiled output)", () => {
  it("reset.css compiles inside @layer base", async () => {
    ...
    // The reset's signature rule (src/styles/css/reset.css).
  ...
  it("compiles Tailwind utilities (built-in and custom @utility) inside @layer utilities", async () => {
    ...
    // A custom @utility from src/styles/css/tailwind.css — proves custom
  ...
  it("root.css/global.css stay unlayered — outside both @layer base and @layer utilities", async () => {
    ...
    const insideBase = globalIndex >= base.bodyStart && globalIndex < base.bodyEnd
    const insideUtilities = globalIndex >= utilities.bodyStart && globalIndex < utilities.bodyEnd
  ...
  it("only two named layers exist — nothing accidentally landed in a third", async () => {
    ...
    const namedLayerBlocks = (css.match(/@layer\s+([a-z]+)\s*\{/gi) ?? []).filter(
      (block) => !block.includes("properties"),
    )
