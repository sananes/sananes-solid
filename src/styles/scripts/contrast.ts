...
export async function readDerivedTokens() {
  const css = readFileSync(join(import.meta.dirname, "..", "css", "global.css"), "utf8")
...
export async function measureContrast(): Promise<Measurement[]> {
  ...
  for (const [name, theme] of Object.entries(themes)) {
    ...
    for (const { token, from, pct, onto } of derived) {
      ...
      resolved.set(token, Color.mix(backdrop, new Color(t[from]), pct, { space: "oklab" }))
    }
...
export const BASELINE_PATH = join(import.meta.dirname, "contrast-baseline.json")
...
export async function readBaseline(): Promise<Baseline> {
  if (!existsSync(BASELINE_PATH)) return { accepted: {}, apcaAccepted: {} }
  const baseline = JSON.parse(readFileSync(BASELINE_PATH, "utf8")) as Partial<Baseline>
  return {
